...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const bigPosterURL =
    bigPosterParam && bigPosterParam !== "null"
      ? bigPosterParam
      : "https://placehold.co/1920x1080/000000/FFFFFF?text=No+Poster"; // Placeholder
  ...
  const smallPosterURL =
    smallPosterParam && smallPosterParam !== "null"
      ? smallPosterParam
      : "https://placehold.co/300x450/000000/FFFFFF?text=No+Poster"; // Placeholder
  ...
  fetch(`https://api.tvmaze.com/seasons/${seasonID}/episodes`)
    ...
    .then(episodes => {
      ...
      episodes.forEach(ep => {
        ...
        img.src = (ep.image && ep.image.medium) ? ep.image.medium : "https://placehold.co/250x140/000000/FFFFFF?text=No+Image"; // Placeholder
...
function loadEpisodeSourceTVMaze(season, episode, title, epName) {
  ...
  // Construct URL based on provider. Note that most providers use structured IDs (IMDb/TMDB, season, episode),
  // rather than a "search as link" approach with episode titles for TV series embeds.
  if (embedProvider === "filmu") {
    ...
    // Player4U explicitly uses a "key" which might be the episode name/title.
  ...
  // Apply a strict sandbox to prevent unwanted navigation, popups, and scripts from the iframe.
  // 'allow-scripts' and 'allow-same-origin' are necessary for some player functionality.
  // 'allow-forms' might be needed if the embed has interactive forms.
  // Crucially, omit 'allow-top-navigation' and 'allow-popups' to prevent the iframe
  // from redirecting the main page or opening new windows/tabs.
  iframe.setAttribute("sandbox", "allow-scripts allow-same-origin allow-forms");

  // The setTimeout to remove the sandbox was removed in the previous iteration,
  // making the sandbox permanent for enhanced security.

  document.getElementById("videoCaption").innerText = `S${season}E${episode} - ${epName}`;
  document.getElementById("videoContainer").style.display = "block";
  document.getElementById("darkOverlay").style.display = "block"; // Ensure overlay is visible
}
...
function loadMovieVideoFromSearch() {
  ...
  if (embedProvider === "filmu") {
    ...
    // Player4U explicitly uses a "key" which might be the movie title.
...
function downloadEpisode(showID, season, episode, title) {
  ...
  window.open(dURL, "_blank"); // This will be blocked by the global window.open override
}
...
function closeVideo() {
  ...
  document.getElementById("videoPlayer").src = ""; // Clear iframe source
  // Restore the overlay and button when closing the video player
  const iframeOverlay = document.getElementById('iframeOverlay');
  const enableIframeBtn = document.getElementById('enableIframeBtn');
  if (iframeOverlay && enableIframeBtn) {
    iframeOverlay.style.display = 'block';
    enableIframeBtn.style.display = 'block';
  }
}

// Trailer and Watchlist functions
document.addEventListener("DOMContentLoaded", function () {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (bookmarkIcon) {
    bookmarkIcon.addEventListener("click", function () {
      ...
      var movie = {
        title: document.getElementById("show-title").textContent.trim(),
        releaseYear: document.getElementById("year-box").textContent.trim(), // Corrected ID from release-year
        duration: document.getElementById("time-box").textContent.trim(), // Corrected ID from duration
        genres: [
          document.getElementById("genre1").textContent.trim(),
          document.getElementById("genre2").textContent.trim()
        ],
        cast: document.getElementById("cast-list").textContent.trim(), // Changed from Array.from to direct textContent
        synopsis: document.getElementById("synopsis").textContent.trim(),
        poster: document.getElementById("smallPoster").src // Corrected ID from poster
      };
      ...
      if (!exists) {
        watchlist.push(movie);
        localStorage.setItem("watchlist", JSON.stringify(watchlist));
        // Using custom toast notification instead of Swal.fire
        document.querySelector("#toast-container") && document.querySelector("#toast-container").showToast('Movie added to watchlist!', 'success');
      } else {
        // Using custom toast notification instead of Swal.fire
        document.querySelector("#toast-container") && document.querySelector("#toast-container").showToast('Movie already exists in your watchlist.', 'info');
      }
    });
  }
  ...
  if (trailerIcon) {
    trailerIcon.addEventListener("click", function () {
      ...
      // NOTE: Ensure you have a valid YouTube Data API key for this to work.
      var apiKey = "AIzaSyBTpP0-7enNqizPGetb_2G5Km_pKdguNF8"; // Replace with your actual YouTube API Key
  ...
  if (typeof YT === "undefined" || typeof YT.Player === "undefined") {
    ...
    tag.src = "https://www.youtube.com/iframe_api?key=AIzaSyBTpP0-7enNqizPGetb_2G5Km_pKdguNF8"; // Replace with your actual YouTube API Key
...
iframe.addEventListener('load', () => {
  // Try to access iframe contentDocument. It might be blocked by CORS for cross-origin iframes.
  try {
    ...
    iframeDoc.addEventListener('click', (e) => {
      ...
      while (target && target !== iframeDoc.body) {
        if (target.tagName === 'A') {
          ...
          break; // Stop traversing if an <a> tag is found
        }
        target = target.parentElement;
      }
    });
  } catch (e) {
    console.warn("Could not attach click listener to iframe due to CORS or security restrictions:", e);
  }
});
...
function isAllowedLink(url) {
  ...
  if (url) {
    for (const allowed of allowedPages) {
      if (url.includes(allowed)) { // Use .includes() for more flexible matching
        return true;
      }
    }
    ...
    // Allow relative paths within your application (e.g., /about, /contact)
...
document.addEventListener('touchend', (e) => {
  ...
  if (tapInterval > 0 && tapInterval < 300 && e.targetTouches.length === 0) { // Check for no active touches
    if (navVisible) {
      nav.style.transform = 'translateY(100%)';
      navVisible = false;
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (path === '') {
    path = 'index.html'; // Assuming index.html is the home page
  }
...
// This overlay intercepts all clicks/touches and blocks them initially
const iframeOverlay = document.getElementById('iframeOverlay');
const enableIframeBtn = document.getElementById('enableIframeBtn');

if (iframeOverlay && enableIframeBtn) {
  // Initial state: overlay is active, blocking clicks
  iframeOverlay.style.display = 'block';
  enableIframeBtn.style.display = 'block'; // Show the enable button

  iframeOverlay.addEventListener('click', function (e) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Blocked an interaction on the iframe via overlay.");
  });

  iframeOverlay.addEventListener('touchstart', function (e) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Blocked a touch interaction on the iframe via overlay.");
  });

  // Optional: Provide a button to allow user interaction (i.e. remove the overlay)
  enableIframeBtn.addEventListener('click', function () {
    iframeOverlay.style.display = 'none'; // Hide the overlay
    enableIframeBtn.style.display = 'none'; // Hide the button
    console.log("User enabled interaction with the iframe.");
  });
} else {
  console.warn("iframeOverlay or enableIframeBtn not found. Iframe interaction control might be limited.");
}
