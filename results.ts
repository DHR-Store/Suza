...
function fetchMoviesOMDb(provider) {
    return fetch(`${BASE_URL}?s=${encodeURIComponent(query)}&page=1&apikey=${API_KEY}`)
        .then(response => response.json())
        .then(results => results.Response === "True" ? results.Search.slice(0, 6) : []);
}

// Fetch movies from IMDb JSON suggestions API and enhance them using OMDb details
    ...
    return fetch(imdbUrl)
        ...
        .then(data => {
            if (data && data.d && data.d.length > 0) {
                ...
                // For each IMDb movie, fetch OMDb details to update image and year if available
                return Promise.all(imdbResults.map(movie => {
                    return fetch(`${BASE_URL}?i=${movie.imdbID}&apikey=${API_KEY}&plot=short`)
                        .then(response => response.json())
                        .then(details => {
...
async function loadMovies() {
    for (const provider of providers) {
        ...
        const [omdbMovies, imdbMovies] = await Promise.all([
            fetchMoviesOMDb(provider),
            fetchMoviesIMDb()
        ]);
        // Combine the two arrays; you can adjust the order as needed.
